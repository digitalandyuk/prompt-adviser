...
- Jev was not called. The helper's request and response were checked against a mocked Decisions API, without a network call or real key. No `OPENROUTER_API_KEY` is available in this Codex process. The macOS Keychain route was added but cannot be exercised until the user creates and stores a key. A live trial needs that key, available account credit and approval for the specific state transmitted.
